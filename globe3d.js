...
//  v2.1 — Fixed: lazy init, unified panel wiring, resizeGlobe()
// ═══════════════════════════════════════════════════════════
...
(function() {
  ...
  function heatColor(i) {
    if (i >= 90) return 0x16a34a;
    if (i >= 75) return 0x4ade80;
    if (i >= 60) return 0xf59e0b;
    if (i >= 45) return 0x0ea5e9;
    if (i >= 25) return 0x6366f1;
    return 0x334155;
  }
  ...
  function onCanvasClick(e) {
    ...
    if (hits.length) {
      ...
      // ← wire to the UNIFIED panel (defined in index.html)
  ...
  function onTouchMove(e) {
    ...
    if(e.touches.length===2){
      ...
      camDist=Math.max(1.5,Math.min(5.5,camDist-(d-lastTouchDist)*.008));
      updateCameraPos(); lastTouchDist=d;
    }
    e.preventDefault();
  }
  ...
  function animate() {
    ...
    glowGroup.children.forEach(m => {
      if (m.userData && m.userData.isPulse) {
        ...
        m.scale.setScalar(s);
        m.material.opacity = .3 + .2*Math.sin(clock.t*3);
      }
    });
  ...
  // ── Resize — called by switchView('3d') in index.html ─
  window.resizeGlobe = function() {
    if (!renderer) return;
    const canvas = renderer.domElement;
    // canvas may have been display:none until now — force layout read
  ...
  // ── Init — lazy, fires when canvas becomes visible ───
  function initGlobe() {
    if (globeInited) return;
    const canvas = document.getElementById('globeCanvas');
    if (!canvas || typeof THREE === 'undefined') return;

    // Set canvas dimensions BEFORE building renderer
    ...
    function waitForDataset(tries) {
      if (window.dataset && Object.keys(window.dataset).length > 0) {
        ...
        animate();
        // Show top country in unified panel
  ...
  // Init on DOMContentLoaded — but canvas is hidden at first,
  // so we ALSO expose initGlobe for switchView('3d') to call.
  window.initGlobe = initGlobe;

  document.addEventListener('DOMContentLoaded', () => {
    // Small delay so app.js finishes loading dataset
